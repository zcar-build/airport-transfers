...
if ( ! defined( 'TRANSFERS_METABOXES_DIR' ) )
	define( 'TRANSFERS_METABOXES_DIR', get_stylesheet_directory_uri() . '/includes/plugins/metaboxes' );

if (!function_exists('transfers_in_array_of_terms')) {
	function transfers_in_array_of_terms($array, $hierarchical, $term_id, $slug) {
		$item = null;
 ...
 */
if (!function_exists('transfers_custom_meta_box_field')) {
	function transfers_custom_meta_box_field( $field, $meta = null, $repeatable = null ) {
		...
		switch( $type ) {
			...
			case 'tax_select':
				...
				foreach ( $terms as $term ) {
					$selected = transfers_in_array_of_terms($post_terms, $taxonomy->hierarchical, $term->term_id, $term->slug); 
			...
			case 'tax_checkboxes':
				...
				foreach ( $terms as $term ) {
					$checked = transfers_in_array_of_terms($post_terms, $taxonomy->hierarchical, $term->term_id, $term->slug); 
			...
			// image
			case 'image':
				$image = TRANSFERS_METABOXES_DIR . '/images/image.png';	
			...
			case 'repeatable':
				...
				foreach( $meta as $row ) {
					...
					foreach ( $repeatable_fields as $repeatable_field ) {
						...
						echo transfers_custom_meta_box_field( $repeatable_field, $meta[$i][$repeatable_field['id']], array( $id, $i ) );
						echo '</p>';
					} // end each field
 ...
 */
if (!function_exists('transfers_meta_box_find_field_type')) {
	function transfers_meta_box_find_field_type( $needle, $haystack ) {
		foreach ( $haystack as $h )
			if ( isset( $h['type'] ) && $h['type'] == 'repeatable' )
				return transfers_meta_box_find_field_type( $needle, $h['repeatable_fields'] );
 ...
 * @return	bool				whether or not the type is in the provided array
 */
if (!function_exists('transfers_meta_box_find_repeatable')) {
	function transfers_meta_box_find_repeatable( $needle = 'repeatable', $haystack ) {
		foreach ( $haystack as $h )
 ...
 */
if (!function_exists('transfers_meta_box_santitize_boolean')) {
	function transfers_meta_box_santitize_boolean( $string ) {
 ...
 */
if (!function_exists('transfers_meta_box_sanitize')) {
	function transfers_meta_box_sanitize( $string, $function = 'sanitize_text_field' ) {
		$allowed_html = array(
			'a' => array(
				'href' => true,
				'title' => true,
 ...
 */
if (!function_exists('transfers_meta_box_array_map_r')) {
	function transfers_meta_box_array_map_r( $func, $meta, $sanitizer ) {
		...
		foreach( $meta as $key => $array ) {
			...
			foreach( $array as $arrayKey => $arrayValue )
				if ( is_array( $arrayValue ) )
					$array[$arrayKey] = transfers_meta_box_array_map_r( $func, $arrayValue, $newSanitizer[$arrayKey] );
 ...
 */
if (!class_exists('Transfers_Add_Meta_Box')) { 

	class Transfers_Add_Meta_Box {
		...
		function admin_enqueue_scripts() {
			...
			if ( in_array( $pagenow, array( 'post-new.php', 'post.php' ) ) && in_array( get_post_type(), $this->page ) ) {
				// js
				$deps = array( 'jquery' );
				if ( transfers_meta_box_find_field_type( 'date', $this->fields ) )
					$deps[] = 'jquery-ui-datepicker';
				if ( transfers_meta_box_find_field_type( 'slider', $this->fields ) )
					$deps[] = 'jquery-ui-slider';
				if ( transfers_meta_box_find_field_type( 'color', $this->fields ) )
					$deps[] = 'farbtastic';
				if ( in_array( true, array(
					transfers_meta_box_find_field_type( 'chosen', $this->fields ),
					transfers_meta_box_find_field_type( 'post_chosen', $this->fields )
				) ) ) {
					wp_register_script( 'chosen', TRANSFERS_METABOXES_DIR . '/js/chosen.js', array( 'jquery' ) );
					$deps[] = 'chosen';
					wp_enqueue_style( 'chosen', TRANSFERS_METABOXES_DIR . '/css/chosen.css' );
				}
				if ( in_array( true, array( 
					transfers_meta_box_find_field_type( 'date', $this->fields ), 
					transfers_meta_box_find_field_type( 'slider', $this->fields ),
					transfers_meta_box_find_field_type( 'color', $this->fields ),
					transfers_meta_box_find_field_type( 'chosen', $this->fields ),
					transfers_meta_box_find_field_type( 'post_chosen', $this->fields ),
					transfers_meta_box_find_repeatable( 'repeatable', $this->fields ),
					transfers_meta_box_find_field_type( 'image', $this->fields ),
					transfers_meta_box_find_field_type( 'file', $this->fields )
				) ) )
					wp_enqueue_script( 'meta_box', TRANSFERS_METABOXES_DIR . '/js/scripts.js', $deps );
				
				// css
				$deps = array();
				wp_register_style( 'jqueryui', TRANSFERS_METABOXES_DIR . '/css/jqueryui.css' );
				if ( transfers_meta_box_find_field_type( 'date', $this->fields ) || transfers_meta_box_find_field_type( 'slider', $this->fields ) )
					$deps[] = 'jqueryui';
				if ( transfers_meta_box_find_field_type( 'color', $this->fields ) )
					$deps[] = 'farbtastic';
				wp_enqueue_style( 'meta_box', TRANSFERS_METABOXES_DIR . '/css/meta_box.css', $deps );
			}
		}
		...
		function admin_head() {
			...
			if ( in_array( get_post_type(), $this->page ) && ( transfers_meta_box_find_field_type( 'date', $this->fields ) || transfers_meta_box_find_field_type( 'slider', $this->fields ) ) ) {
		...
		function meta_box_callback() {
			...
			wp_nonce_field( 'transfers_plugin_meta_box_nonce_action', 'transfers_plugin_meta_box_nonce_field' );
			...
			foreach ( $this->fields as $field) {
				if ( $field['type'] == 'section' ) {
					...
					echo '<tr>
							...
							echo transfers_custom_meta_box_field( $field, $meta );
		...
		function save_box( $post_id ) {
			...
			// verify nonce
			if ( ! isset( $_POST['transfers_plugin_meta_box_nonce_field'] ) )
				return $post_id;
			if ( ! ( in_array( $post_type, $this->page ) || wp_verify_nonce( $_POST['transfers_plugin_meta_box_nonce_field'],  'transfers_plugin_meta_box_nonce_action' ) ) ) 
				return $post_id;
			// check autosave
			...
			foreach ( $this->fields as $field ) {
				...
				if( in_array( $field['type'], array( 'tax_select', 'tax_checkboxes' ) ) ) {
					...
					if ( isset( $new ) && '' == $new && $old ) {
						...
						if ( is_array( $new ) )
							$new = transfers_meta_box_array_map_r( 'transfers_meta_box_sanitize', $new, $sanitizer );
						else
							$new = transfers_meta_box_sanitize( $new, $sanitizer );
